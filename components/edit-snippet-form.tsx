...
import { X, Copy, Check, Wand2, Eye, EyeOff, Save } from "lucide-react";
...
// Modified form schema with properly typed fields
...
export const EditSnippetForm = ({ user, snippet }: EditSnippetFormProps) => {
  ...
  // Use the proper form schema and populate it with existing snippet data
  ...
  // Modified onSubmit function with proper type handling
  const onSubmit = async (data: FormValues) => {
    try {
      // Ensure we have proper tags array
      const processedTags = Array.isArray(data.tags) ? data.tags : [];
  ...
  // Modified addTag function with proper type handling
  ...
  // Modified removeTag function with proper type handling
  ...
  // If somehow we got here without a valid snippet
  if (!snippet || !snippet.id) {
    return (
      <div className="p-4 bg-destructive/10 rounded-md text-destructive">
        <p>Error: Unable to load snippet data. Please go back and try again.</p>
        <Button 
          variant="outline" 
